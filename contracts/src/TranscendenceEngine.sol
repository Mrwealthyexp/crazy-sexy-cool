...
contract TranscendenceEngine is Ownable, ReentrancyGuard {
    ...
    struct SoulState {
        ...
        uint256 grayKarma;
        uint256 soulTokens;
        mapping(uint256 => uint256) jobLevels;
    ...
    function initializeSoulState(uint256 soulId) external onlySoulOwner(soulId) {
        ...
        soul.stage = startingState;
        soul.whiteKarma = 0;
        soul.blackKarma = blueprint.karmicDebt;
        soul.grayKarma = 0;
        soul.soulTokens = 100 * 10 ** 18;
        soul.mitochondrialPoints = 0;
        soul.overEvolved = false;
        soul.lastAscensionTime = 0;
        soul.interventionsPerformed = 0;
        soul.interventionsReceived = 0;
    ...
    function executeWeatherChange(uint256, uint256 zoneId, bytes32 zkProof) internal {
        ...
        zoneWeatherHistory[zoneId].push(
            WeatherEffect({
                zoneId: zoneId,
        ...
        )
    {
        ...
        return (
            ...
            soul.grayKarma,
            soul.soulTokens,
            soul.mitochondrialPoints,
            soul.overEvolved,
            soul.lastAscensionTime,
            soul.interventionsPerformed,
            soul.interventionsReceived
        );
    }

    function getJobLevel(uint256 soulId, uint256 jobId) external view returns (uint256) {
        return soulStates[soulId].jobLevels[jobId];
    }
